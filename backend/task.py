...
# def fetch_story_and_comments(session, story_id):
...
# async def fetch_top_stories_and_comments():
#     existing_story_ids = set(Story.objects.values_list('story_id', flat=True))
...
#             response = await session.get(top_stories_url)
#             if response.status_code == 200:
#                 new_stories_ids = response.json()

#                 new_stories = [story_id for story_id in new_stories_ids if story_id not in existing_story_ids]

#                 story_tasks = []
#                 for story_id in new_stories[:100]:
#                     story_tasks.append(fetch_story_and_comments(session, story_id))
